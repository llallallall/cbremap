$(document).ready(function($) {
    ...
    if ($("#" + mapId).length) {
        ...
        // MAP ELEMENT
        // =================================================================================================================
        var grayscale = L.tileLayer(leafletMapProvider, {id: 'mapbox/light-v9', accessToken: mapBoxAccessToken,attribution: leafletAttribution});
        var streets = L.tileLayer(leafletMapProvider, {id: 'mapbox/streets-v11', accessToken: mapBoxAccessToken,attribution: leafletAttribution});
        var satellite = L.tileLayer(leafletMapProvider, {id: 'mapbox/satellite-streets-v11', accessToken: mapBoxAccessToken,attribution: leafletAttribution});
        ...
        //         iconAnchor: [17, 51]
        //     });
        //     var currentLocMarker = L.marker(e.latlng,{
        //         icon: redMarkerIcon,
        //         title: e.latlng,
        //         draggable: markerDrag
        //     }).addTo(map).on('click', markerDelAgain);;

        //     marker.push(currentLocMarker);
            ...
            resultsHtml.push(
                '<div class="ts-result-link" data-ts-id="' + loadedMarkersData[id]["id"] + '" data-ts-ln="' + newMarkers[id].loopNumber + '">' +
                    ...
                    '<a href="' + loadedMarkersData[id]["url"] + '" class="card ts-item ts-card ts-result">' +
                        ( ( loadedMarkersData[i]["ribbon"] !== undefined ) ? '<div class="ts-ribbon">' + loadedMarkersData[i]["ribbon"] + '</div>' : "" ) +
                        ( ( loadedMarkersData[i]["ribbon_corner"] !== undefined ) ? '<div class="ts-ribbon-corner"><span>' + loadedMarkersData[i]["ribbon_corner"] + '</span></div>' : "" ) +
                        '<div href="detail-03.html" class="card-img ts-item__image" style="background-image: url(' + loadedMarkersData[id]["marker_image"] + ')"></div>' +
                        '<div class="card-body">' +
